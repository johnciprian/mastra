...
import { assertFactoryAuthConfig, isAuthDisabled, resolveFactoryPublicUrl } from './auth-config.js';
import type { FactoryAuthConfig } from './auth-config.js';
...
export interface MastraFactoryConfig {
  /**
   * REQUIRED. Exactly two kinds of value are accepted, and the choice between
   * them is always written down at the call site:
   *
   * 1. An auth provider instance — `MastraAuthWorkos` (`@mastra/auth-workos`),
   *    `MastraAuthBetterAuth` (`@mastra/auth-better-auth`), the result of
   *    `createMastraPlatformAuth({ publicUrl })` for identity proxied through
   *    the shared Mastra platform API, or any custom `MastraAuthProvider`.
   *    Whatever instance is passed is the active provider, honored as-is.
   * 2. `AUTH_DISABLED` — an open server with no authentication at all
   *    (local-dev behavior).
   *
   * There is no third state. Omitting the slot no longer selects a default
   * provider, and `null` no longer disables auth; both now throw from the
   * constructor. `createMastraPlatformAuth({ publicUrl })` is what omission
   * used to give you — pass it explicitly if that is what you want. The point
   * is that a deployment that forgot to configure auth can no longer boot
   * looking healthy while pointed at the platform's shared identity API.
   */
...
export class MastraFactory {
  ...
  constructor(config: MastraFactoryConfig) {
    ...
    // Fail at construction, not at prepare(): a misconfigured `auth` slot is a
    // deployment mistake, and the earlier it surfaces the less of the boot has
    // already run. This also catches the JS callers TypeScript never sees.
    assertFactoryAuthConfig(config.auth);
    this.#config = config;
  }
  ...
  async prepare(): Promise<MastraArgs> {
    ...
    const publicOrigin = resolveFactoryPublicUrl(this.#config.publicUrl);
    ...
    // One value in, one value out. `auth` is `undefined` from here down when and only
    // when the config said AUTH_DISABLED; there is no default to fall into, and no
    // value that means one thing above this line and another below it.
    const configured = this.#config.auth;
    const auth: IMastraAuthProvider | undefined = isAuthDisabled(configured) ? undefined : configured;
