...
Branching on the name is what gives an unrecognized provider a GitHub logo and a "Continue with GitHub" button: the screen has no other question it knows how to ask.
...
**`signIn.signUpEnabled` is positive, and it answers `true` only for a literal `true`.** `isSignUpEnabled` is declared synchronous, but nothing stops a provider writing `async isSignUpEnabled()` — and an `async` method returns a Promise, which is truthy. A loose reader would show a sign-up link on a deployment that had switched sign-up off, and nothing would look broken to anyone. An absent method still means "sign-up is on", which is the contract's documented default; a method that answers a Promise, a throw, or anything else means `false`. A rejecting Promise is judged without being awaited and without leaving an unhandled rejection behind.
