...
`@mastra/factory-auth/contract` re-exports the Mastra auth provider contract, so a provider author has one import path instead of hunting through `@mastra/core`.
