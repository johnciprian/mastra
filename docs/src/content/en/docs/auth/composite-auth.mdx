...
```typescript prettier:false docscheck=skip docscheck-reason="pseudocode of the internal loop, not an API a reader calls"
...
```typescript prettier:false docscheck=skip docscheck-reason="a two-line ordering illustration, reusing providers built under an earlier heading"
