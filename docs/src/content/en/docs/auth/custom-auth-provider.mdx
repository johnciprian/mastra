...
Implementing the contract isn't sufficient on its own. A provider can typecheck cleanly and still fail to run a deployment, because some requirements sit outside the interfaces entirely. Read [Provider obligations](/docs/auth/provider-obligations) before you publish a provider.

For a build that starts from an empty file and ends with a passing conformance suite, follow [Write an auth provider](/docs/auth/write-a-provider).
