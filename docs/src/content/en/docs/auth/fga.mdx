...
```typescript prettier: false title="src/mastra/index.ts"
...
const mastra = new Mastra({
  server: {
    auth: new MastraAuthWorkos({
      ...
      mapUserToResourceId: user => user.organizationId,
    }),
...
```typescript prettier: false title="src/mastra/index.ts" docscheck=statements
new MastraFGAWorkos({
  resourceMapping: {
...
```typescript prettier: false title="src/mastra/index.ts" docscheck=replace
import { Mastra } from '@mastra/core/mastra';
  ...
  server: {
    storedResources: {
      scope: {
        ...
        resolve: ({ user }) => (user as { organizationId?: string }).organizationId,
        requireScope: true,
      },
    },
  },
...
```typescript prettier: false title="src/routes/projects.ts"
...
const routeFGA: Record<string, FGARouteConfig> = {
...
```typescript title="src/fga-provider.ts" docscheck=member docscheck-scaffold=fga-provider
import { FGADeniedError } from '@mastra/core/auth/ee'
import type { ActorSignal, FGACheckParams } from '@mastra/core/auth/ee'
...
```typescript docscheck=statements docscheck-scaffold=fga-actors
await run.start({
