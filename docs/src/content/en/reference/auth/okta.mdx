...
To use Okta RBAC with a different auth provider, pass a `getUserId` function to resolve the Okta user ID from the other provider's user object. `getUserId` receives `unknown`, because the user shape belongs to whichever provider authenticated the request, so narrow it to the shape that provider returns:
...
export const mastra = new Mastra({
  server: {
    ...
    rbac: new MastraRBACOkta({
      getUserId: user => {
        const profile = user as Auth0Profile
        return profile.metadata?.oktaUserId ?? profile.email
      },
      roleMapping: {
