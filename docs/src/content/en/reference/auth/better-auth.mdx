...
```typescript title="src/mastra/better-auth-user.ts"
...
- `session`: The Better Auth session object containing session metadata, plus `activeOrganizationId` from the organization plugin the provider registers. When the stored session has none, `authenticateToken` resolves a default from the user's memberships.
