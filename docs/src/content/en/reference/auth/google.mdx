...
```typescript docscheck=statements docscheck-scaffold=google-reference
...
```typescript title="src/mastra/index.ts" docscheck=replace
import { Mastra } from '@mastra/core'
...
To use Google Workspace RBAC with a different auth provider, pass a `getUserKey` function to resolve the Google Directory API user key from the other provider's user object. `getUserKey` receives `unknown`, because the user shape belongs to whichever provider authenticated the request, so narrow it to the shape that provider returns:
...
export const mastra = new Mastra({
  server: {
    ...
    rbac: new MastraRBACGoogle({
      getUserKey: user => (user as { email?: string }).email,
      serviceAccount: {
        clientEmail: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL!,
        privateKey: process.env.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY!,
        subject: process.env.GOOGLE_WORKSPACE_ADMIN_EMAIL!,
